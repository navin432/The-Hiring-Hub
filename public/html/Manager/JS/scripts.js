...
const employees = {
    emp1: { name: 'John Doe', role: 'Software Engineer', ratings: { productivity: 0, teamwork: 0, punctuality: 0, innovation: 0 }, remarks: '' },
    emp2: { name: 'Jane Smith', role: 'Marketing Specialist', ratings: { productivity: 0, teamwork: 0, punctuality: 0, innovation: 0 }, remarks: '' },
    emp3: { name: 'Alice Brown', role: 'HR Manager', ratings: { productivity: 0, teamwork: 0, punctuality: 0, innovation: 0 }, remarks: '' },
    emp4: { name: 'Chris Evans', role: 'Data Analyst', ratings: { productivity: 0, teamwork: 0, punctuality: 0, innovation: 0 }, remarks: '' },
    emp5: { name: 'Emily Davis', role: 'Product Manager', ratings: { productivity: 0, teamwork: 0, punctuality: 0, innovation: 0 }, remarks: '' },
  };
  ...
  categories.forEach((category) => {
    const starContainer = document.querySelector(`.stars[data-category="${category}"]`);
  ...
  document.getElementById('employeeSelector').addEventListener('change', (event) => {
    const selectedEmployee = event.target.value;
  
    // Save current employee remarks
    employees[currentEmployee].remarks = document.getElementById('remarks').value;
  ...
  function updateChart() {
    ...
    performanceChart.data.labels.push(`Update ${performanceChart.data.labels.length + 1}`);
